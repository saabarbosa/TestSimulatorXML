...
namespace ManipulaXMLCulturaInglesa
{
    public class LerXML
    {
        ...
        public static void LerXMLPergResp(String _caminho, Boolean _junior, out DataTable _dtPerguntas, out DataTable _dtRespostas)
        {
            ...
            _dtPerguntas.Columns.Add("Pictures");
            ...
            if (!_junior)
            {
                ...
                foreach (XmlNode no1 in nos.ChildNodes)
                {
                    ...
                    {
                        case "Question":
                            _dtPerguntas.Rows.Add(no1.Attributes["Value"].Value, no1.Attributes["Picture"].Value);
                            //_dtPerguntas.Rows.Add(no1.Attributes["Picture"].Value);
                            NumPergunta++;
                            break;
                    }
